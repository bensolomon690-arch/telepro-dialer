...
# --- 4. NAVIGATION ---
st.sidebar.title("Telecall Pro v6.0")
...
if nav == "📊 Stats":
    ...
    if not df.empty:
        ...
        # Now correctly counts 'Completed' and 'Bending'
        c2.metric("Completed", len(df[df['Status'] == 'Completed']))
        c3.metric("Bending/Pending", len(df[df['Status'].isin(['Bending', 'Pending'])]))
    ...
    if up:
        ...
        # SMART MAPPING: Specifically looks for your office 'CURRENT STATUS'
        map_cols = {
            'CLIENT NAME':'Name', 'NAME':'Name', 
            'CLIENT CODE':'ID', 'MOBILE NUMBER':'Number', 
            'NUMBER':'Number', 'CURRENT STATUS':'Status', 'STATUS':'Status'
        }
        new_df = new_df.rename(columns=map_cols)
        
        # Ensure 'Status' is readable and 'Notes' exist
        ...
        if st.button("🔥 Finalize Sync"):
            ...
            st.success("Master Database Updated with Bending/Completed data.")

elif nav == "🎯 Call Station":
    st.header("🎯 Active Dialer Terminal")
    df = load_db()
    if not df.empty:
        # Filter Bar including 'Bending'
        status_filter = st.radio("Select List", ["All", "Pending", "Bending", "Follow-up", "Completed"], horizontal=True)
        ...
        edited_df = st.data_editor(display_df, use_container_width=True, num_rows="dynamic", key="v6_editor")
        
        if st.button("💾 SAVE ALL PROGRESS"):
            # This locks in your 'Completed' or 'Bending' status changes
            df.update(edited_df)
            df.to_csv(DB_FILE, index=False)
            st.success("Changes permanently saved! Check the Stats page now.")
